...
def yield_fun2(E, costheta, Emax, del_max, R0, E0, s, fact_theta):
    
    del_max_tilde=del_max*exp(fact_theta*(1.-costheta));
...
class SEY_model_ECLOUD:
    def __init__(self, Emax,del_max,R0,E0=150., s=1.35, fact_theta=0.5):
            self.Emax = Emax
            self.del_max = del_max
            self.R0 = R0
            self.E0 = E0
            self.s = s
            self.fact_theta = fact_theta
            
            print 'Secondary emission model: ECLOUD E0=%.4f s=%.4f'%(self.E0, self.s)
            
    def SEY_process(self,nel_impact,E_impact_eV, costheta_impact, i_impact):
            yiel, ref_frac=yield_fun2(E_impact_eV,costheta_impact,self.Emax,self.del_max,self.R0, E0=self.E0, s=self.s, fact_theta=self.fact_theta);
